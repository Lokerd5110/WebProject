...
let renderWatched = (data) => {
  ...
  data.forEach((item) => {
    if (item.titleType == 'anime') {
      let episodes = item.titleType == 'anime' ? '2 эпизод' : 'Манга';
      let stopped = item.titleType == 'anime' ? '13:45' : '7 глава';
...
let render = (data) => {
  ...
  for (let i = 0; i < genreTitles.length; i++) {
    ...
    for(let j = 0; j < cardList.length; j++) {
      ...
      cardList[j].querySelector(".episodes").innerHTML = data[j].episodes + ' эпизодов';
